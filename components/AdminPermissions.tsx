        ...
        <div className="flex flex-col h-full bg-[#F8FAFC] dark:bg-gray-950 transition-colors duration-300">
            <div className="bg-white dark:bg-gray-900 p-4 border-b dark:border-gray-800 flex items-center justify-between sticky top-0 z-40">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={handlePersistChanges} disabled={!hasChanges} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${hasChanges ? 'bg-primary text-white shadow-xl shadow-green-500/20 hover:scale-105' : 'bg-gray-200 dark:bg-gray-800 text-gray-400 cursor-not-allowed'}`}><Save size={18} /> Sauvegarder</button>
                </div>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="w-80 bg-white dark:bg-gray-900 border-r dark:border-gray-800 flex flex-col shrink-0">
                    <div className="p-6">
                        <button onClick={() => { setEditingRoleId(null); setRoleNameInput(''); setShowRoleModal(true); }} className="w-full py-4 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-3xl text-gray-400 hover:border-primary hover:text-primary transition-all flex items-center justify-center gap-2 font-black uppercase text-[10px] tracking-widest"><Plus size={16} /> Nouveau Rôle</button>
                    </div>
                    <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-2 no-scrollbar">
                        {roles.map(role => (
                            <div key={role.id} onClick={() => setSelectedRoleId(role.id)} className={`group p-4 rounded-2xl cursor-pointer transition-all flex items-center justify-between border-2 ${selectedRoleId === role.id ? 'bg-primary/5 border-primary text-primary' : 'border-transparent text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                                <div className="flex items-center gap-4 overflow-hidden">
                                    <div className={`p-2 rounded-xl shrink-0 ${selectedRoleId === role.id ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-gray-800'}`}><role.icon size={18} /></div>
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                            ...
                            <div className="relative w-full md:w-64">
                                ...
                                <input type="text" placeholder="Filtrer permissions..." className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white dark:bg-gray-900 border-none text-xs font-bold shadow-sm outline-none focus:ring-2 ring-primary/20" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                            </div>
                        </div>
                                ...
                                <div key={category} className="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden animate-fade-in-up">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x dark:divide-gray-800">
                                        {filtered.map(perm => {
                                            ...
                                            return (
                                                <div key={perm.key} onClick={() => !isLocked && handleTogglePermission(perm.key)} className={`p-6 flex items-start gap-4 transition-all cursor-pointer group ${isActive ? 'bg-primary/[0.02]' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'} ${isLocked ? 'opacity-60 cursor-not-allowed' : ''}`}>
                                                    <div className={`mt-1 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all shrink-0 ${isActive ? 'bg-primary border-primary text-white' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>{isActive && <Check size={14} strokeWidth={4} />}</div>
                    ...
                    <form onSubmit={handleSaveRole} className="bg-white dark:bg-gray-950 rounded-[3rem] w-full max-w-sm p-8 relative z-10 shadow-2xl border dark:border-gray-800 animate-scale-up">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input autoFocus required className="w-full p-5 bg-gray-50 dark:bg-gray-900 rounded-2xl border-none outline-none font-black text-sm dark:text-white focus:ring-2 ring-primary/20" placeholder="ex: Superviseur Zone Est" value={roleNameInput} onChange={e => setRoleNameInput(e.target.value)} />
                            </div>
                            <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-2xl border border-blue-100 dark:border-blue-900/30 flex gap-3">
                                <Info size={18} className="text-blue-500 shrink-0 mt-0.5" /><p className="text-[10px] text-blue-600 dark:text-blue-300 font-bold leading-relaxed uppercase tracking-tight">Une fois créé, vous pourrez assigner des permissions spécifiques à ce rôle dans la matrice.</p>
                            </div>
                            <button type="submit" className="w-full py-5 bg-primary text-white rounded-[1.5rem] font-black uppercase tracking-widest shadow-xl shadow-green-500/20 active:scale-95 transition-all">{editingRoleId ? 'Mettre à jour' : 'Créer le rôle'}</button>
                        </div>
                    </form>
                </div>
            )}
