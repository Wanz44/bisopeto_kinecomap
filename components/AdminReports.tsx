...
export const AdminReports: React.FC<AdminReportsProps> = ({ onBack, onToast, onNotify, currentUser }) => {
    ...
    const getSLADelay = (date: string) => {
        ...
        if (diffHours < 1) return { text: "Récent", color: "text-green-500", level: 'low' };
        if (diffHours < 24) return { text: `${diffHours}h`, color: "text-orange-500", level: 'med' };
        return { text: `${Math.floor(diffHours/24)}j`, color: "text-red-500 font-black", level: 'crit' };
    };
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`text-[9px] font-black uppercase ${sla.color} flex items-center gap-1 shrink-0 ml-2`} title="Temps écoulé depuis signalement">
                                                <Timer size={10}/> {sla.text}
                                            </span>
                                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1.5 flex items-center gap-1.5">
                                        <Timer size={10} className="text-blue-500"/> SLA STATUS: {getSLADelay(selectedReport.date).text.toUpperCase()}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar pb-32">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl h-80 border-4 border-white dark:border-gray-800 bg-gray-100">
                                    <img src={viewMode === 'before' ? selectedReport.imageUrl : selectedReport.proofUrl} className="w-full h-full object-cover" alt="Preuve Terrain" />
