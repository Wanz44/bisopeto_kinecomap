...
export const AdminAds: React.FC<AdminAdsProps> = ({ onBack, onToast }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#F0F2F5] dark:bg-[#050505] transition-colors duration-300 overflow-hidden">
            ...
            {/* Header Spécifique Ad-Center (Différent du Dashboard) */}
            <div className="bg-[#1A237E] dark:bg-gray-900 p-8 text-white relative overflow-hidden shrink-0">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-3xl font-black tracking-tighter uppercase leading-none">Régie Publicitaire</h2>
                            <p className="text-[10px] font-black text-blue-200 uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                                <ShieldCheck size={12}/> Ad-Server Engine Enterprise
                            </p>
                        </div>
                    </div>

                    <button onClick={() => setShowCampaignModal(true)} className="bg-white text-[#1A237E] px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3">
                ...
                <div className="mt-10 flex gap-2 overflow-x-auto no-scrollbar">
                    {[
                        ...
                        <button 
                            ...
                            className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === tab.id ? 'bg-white text-[#1A237E] shadow-xl' : 'bg-white/5 text-blue-100 hover:bg-white/10'}`}
                        >
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 no-scrollbar pb-32">
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center h-64 gap-4">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Chargement de l'inventaire...</p>
                    </div>
                ) : activeTab === 'campaigns' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 animate-fade-in">
                        {ads.length === 0 ? (
                            <div className="col-span-full py-24 text-center bg-white dark:bg-gray-900 rounded-[3rem] border-2 border-dashed border-gray-200 dark:border-gray-800">
                                ...
                                <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Aucune campagne active</p>
                            </div>
                        ) : ads.map(ad => {
                                ...
                                <div key={ad.id} className="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm hover:shadow-xl transition-all group">
                                    <div className="h-44 relative overflow-hidden">
                                        ...
                                        <div className="absolute bottom-4 left-6 right-6">
                                            <h4 className="text-white font-black uppercase text-base truncate leading-none mb-1">{ad.title}</h4>
                                            <p className="text-[10px] text-blue-300 font-bold uppercase tracking-widest flex items-center gap-1.5"><Building2 size={12}/> {ad.partner}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6">
                                        <div className="grid grid-cols-3 gap-3 mb-6">
                                            <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                                                <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Vues</p>
                                                <p className="text-sm font-black dark:text-white">{(ad.views / 1000).toFixed(1)}k</p>
                                            </div>
                                            <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                                                <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Clicks</p>
                                                <p className="text-sm font-black dark:text-white">{ad.clicks}</p>
                                            </div>
                                            <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/10 rounded-2xl">
                                                <p className="text-[8px] font-black text-blue-400 uppercase mb-1">CTR</p>
                                                <p className="text-sm font-black text-blue-600">{ctr}%</p>
                                            </div>
                                        </div>

                                        <div className="flex items-center justify-between pt-4 border-t dark:border-gray-800">
                                            <div className="flex flex-col">
                                                <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Ciblage Géo</span>
                                                <span className="text-[10px] font-black dark:text-white uppercase truncate">{ad.targetCommune === 'all' ? 'Tout Kinshasa' : ad.targetCommune}</span>
                                            </div>
                                            <div className="flex flex-col text-right">
                                                <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Budget Consommé</span>
                                                <span className="text-[10px] font-black text-green-600">${ad.spent.toFixed(1)} / ${ad.budget}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-4">
                        ...
                        <p className="text-xs font-black uppercase tracking-[0.2em]">Module {activeTab} en cours de synchronisation...</p>
                    </div>
                )}
            </div>

            {/* MODAL CRÉATION CAMPAGNE AVEC LIVE SIMULATOR */}
            {showCampaignModal && (
                <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white dark:bg-[#0d1117] rounded-[3rem] w-full max-w-5xl relative z-10 shadow-2xl animate-scale-up border dark:border-gray-800 flex flex-col lg:flex-row overflow-hidden max-h-[95vh]">
                            ...
                            <form onSubmit={handleCreateAd} className="space-y-8">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <select required className="w-full p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none outline-none font-black text-xs dark:text-white shadow-inner" value={adForm.partner} onChange={e => setAdForm({...adForm, partner: e.target.value})}>
                                            <option value="">Sélectionner</option>
                                            {partners.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                                        </select>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Budget Total ($)</label>
                                        <input type="number" required className="w-full p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none outline-none font-black text-xs dark:text-white shadow-inner" value={adForm.budget} onChange={e => setAdForm({...adForm, budget: Number(e.target.value)})} />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input required className="w-full p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none outline-none font-black text-sm dark:text-white shadow-inner" placeholder="ex: Mbote Kinshasa! Profitez de..." value={adForm.title} onChange={e => setAdForm({...adForm, title: e.target.value})} />
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Ciblage Géographique</label>
                                        <select className="w-full p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none font-black text-xs dark:text-white shadow-inner" value={adForm.targetCommune} onChange={e => setAdForm({...adForm, targetCommune: e.target.value})}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select className="w-full p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none font-black text-xs dark:text-white shadow-inner" value={adForm.targetUserType} onChange={e => setAdForm({...adForm, targetUserType: e.target.value as any})}>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Lien Image URL (HD)</label>
                                    <div className="relative">
                                        <ImageIcon size={18} className="absolute left-5 top-5 text-gray-400" />
                                        <input required className="w-full pl-14 pr-5 py-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border-none outline-none font-bold text-xs dark:text-white shadow-inner" placeholder="https://cloud.com/image.jpg" value={adForm.image} onChange={e => setAdForm({...adForm, image: e.target.value})} />
                                    </div>
                                </div>

                                <button disabled={isSaving} className="w-full py-6 bg-[#1A237E] text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
                        ...
                        {/* 2. Simulateur Mobile Live à droite */}
                        <div className="hidden lg:flex w-[450px] bg-gray-100 dark:bg-[#050505] p-12 flex-col items-center justify-center border-l dark:border-gray-800 relative">
                             <div className="absolute top-8 left-10 flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Prévisualisation In-App</span>
                             </div>
                             ...
                             {/* Le téléphone simulé (Design iPhone-ish) */}
                             <div className="w-[300px] h-[600px] bg-white dark:bg-black rounded-[4rem] border-[12px] border-gray-900 dark:border-gray-800 shadow-2xl overflow-hidden relative">
                                ...
                                {/* Contenu simulé de l'appli Biso Peto */}
                                ...
                                {/* L'annonce injectée en temps réel */}
                                <div className="absolute top-1/2 left-4 right-4 -translate-y-1/2">
                                    <div className="flex items-center justify-between px-3 mb-2">
                                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5"><Megaphone size={10} className="text-blue-600"/> Bons Plans {adForm.targetCommune === 'all' ? 'Kin' : adForm.targetCommune}</span>
                                        <span className="text-[7px] font-bold text-gray-300 uppercase">Sponsorisé</span>
                                    </div>
                                    <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden animate-fade-in">
                                        <div className="h-36 bg-gray-50 dark:bg-gray-800 relative">
                                            ...
                                            <div className="absolute bottom-4 left-6 right-6">
                                                <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest mb-1.5">{adForm.partner || 'Votre Marque'}</p>
                                                <h5 className="text-white font-black uppercase text-[13px] leading-tight line-clamp-2">{adForm.title || 'Votre accroche publicitaire apparaîtra ici'}</h5>
                                            </div>
                                        </div>
                                    </div>
                                </div>
