...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onChangeView, onLogout, onRefresh, isRefreshing, user, unreadNotifications = 0, appLogo = 'logobisopeto.png', toast, onCloseToast }) => {
    ...
    const viewsWithInternalHeader = [
        ...
        AppView.ADMIN_CASHBOOK,
        // Public / Landing Views should not have the global dashboard header
        AppView.LANDING,
    ...
    const getNavItems = (isMobile: boolean): NavItem[] => {
        ...
        if (user?.type === UserType.ADMIN) {
            const adminFull = [
                ...
                { view: AppView.ADMIN_CASHBOOK, icon: Wallet, label: 'Livre de Caisse' },
    ...
    return (
        <div className="w-full h-full flex bg-[#F8FAFC] dark:bg-[#050505] transition-colors duration-500 overflow-hidden relative font-sans">
            ...
            {user && (
                <aside className="hidden lg:flex flex-col w-[18rem] h-[calc(100vh-3rem)] m-6 rounded-[2.5rem] bg-white dark:bg-[#111827] border border-gray-100 dark:border-white/5 shadow-2xl transition-all duration-300 relative z-[60] shrink-0">
            ...
            <div className="flex-1 flex flex-col h-full w-full overflow-hidden relative z-10">
                {showGlobalHeader && (
                    <header className="min-h-[4.5rem] md:min-h-[5.5rem] px-6 md:px-10 flex items-center justify-between sticky top-0 z-40 shrink-0 bg-white/90 backdrop-blur-xl dark:bg-black/90 border-b dark:border-white/5 safe-area-pt">
                        <div className="flex items-center gap-3 lg:hidden">
                            <div className="w-9 h-9 bg-white dark:bg-black rounded-lg flex items-center justify-center shadow-md border border-gray-100 p-1"><img src={appLogo} alt="Logo" className="w-full h-full object-contain" /></div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={onRefresh} className={`p-2.5 rounded-xl text-gray-500 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 active:scale-90 transition-all ${isRefreshing ? 'animate-spin' : ''}`}><RefreshCw size={18} /></button>
                            <button onClick={() => onChangeView(AppView.NOTIFICATIONS)} className="p-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 relative active:scale-90 transition-all">
                                <Bell size={18} />
                                {unreadNotifications > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[7px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-black shadow-lg">{unreadNotifications}</span>}
                            </button>
                        </div>
                    </header>
                )}
                ...
                <main className="flex-1 overflow-y-auto overflow-x-hidden relative h-full w-full no-scrollbar transition-all duration-500 scroll-container">
                    <div className={`${!showGlobalHeader ? 'pt-0' : 'pt-0'} ${user ? 'pb-[calc(7.5rem+env(safe-area-inset-bottom))]' : 'pb-0'} lg:pb-8 h-full`}>
                        {children}
                    </div>
                </main>
                
                {user && (
                    <nav className="lg:hidden fixed bottom-6 inset-x-6 h-16 bg-white/95 dark:bg-[#111827]/95 backdrop-blur-2xl border border-gray-100 dark:border-white/10 rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.25)] z-[100] flex items-center justify-evenly px-4 mb-[env(safe-area-inset-bottom)]">
                        {mobileTabItems.map((item) => {
                            ...
                            return (
                                <button key={item.view} onClick={() => onChangeView(item.view)} className="flex flex-col items-center justify-center h-full relative flex-1">
                                    <div className={`relative z-10 p-3.5 rounded-2xl transition-all duration-500 ${isActive ? 'bg-primary text-white -translate-y-5 shadow-2xl scale-110 shadow-primary/40' : 'text-gray-400 hover:text-primary/50'}`}>
                                        <item.icon size={22} />
                                    </div>
            ...
            {/* TOAST SYSTEM AT TOP LAYER */}
            {toast && toast.visible && (
                <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[9999] animate-fade-in-up w-[92%] max-w-sm">
                    <div className={`px-6 py-4 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex items-center gap-4 border backdrop-blur-3xl ${toast.type === 'success' ? 'bg-green-600/90 text-white border-green-400/20' : toast.type === 'error' ? 'bg-red-600/90 text-white border-red-400/20' : 'bg-blue-600/90 text-white border-blue-400/20'}`}>
