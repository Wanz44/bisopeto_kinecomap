    ...
    const getNavSections = (): NavSection[] => {
        ...
        if (type === UserType.ADMIN) {
            const sections: NavSection[] = [
                { items: [{ view: AppView.DASHBOARD, icon: Home, label: 'Dashboard' }] }
            ];
        ...
        <div className="w-full h-[100dvh] flex bg-[#F5F5F5] dark:bg-[#050505] transition-colors duration-500 overflow-hidden relative font-sans">
            ...
            <aside className="hidden md:flex flex-col w-[18rem] h-[calc(100vh-3rem)] m-6 rounded-[2.5rem] bg-white/95 dark:bg-[#111827]/95 backdrop-blur-2xl border border-gray-100 dark:border-white/5 shadow-2xl transition-all duration-300 relative z-50 shrink-0">
                            ...
                            {section.items.map((item) => {
                                ...
                                return (
                                    <button key={item.view} onClick={() => onChangeView(item.view)} className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 group relative ${isActive ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5'}`}>
                                        <item.icon className={`w-5 h-5 relative z-10 ${isActive ? 'scale-110' : 'group-hover:scale-110'}`} />
                ...
                <div className="p-6 mx-2 border-t border-gray-100 dark:border-gray-800">
                    <button onClick={() => setShowLogoutConfirm(true)} className="w-full flex items-center gap-3 p-4 rounded-2xl text-red-500 hover:bg-red-50 transition-colors"><div className="p-2 bg-red-100 rounded-xl"><LogOut className="w-5 h-5" /></div><span className="font-bold text-sm">Déconnexion</span></button>
                </div>
            </aside>
            ...
            <div className="flex-1 flex flex-col h-full w-full overflow-hidden relative z-10">
                <header className="min-h-[5rem] px-6 md:px-8 flex items-center justify-between sticky top-0 z-40 shrink-0 bg-white/50 backdrop-blur-sm dark:bg-black/50">
                    ...
                    <div className="hidden md:block"><h1 className="font-black text-2xl text-gray-800 dark:text-white tracking-tight flex items-center gap-3">{user?.type === UserType.ADMIN && <Shield className="w-6 h-6 text-secondary" />}{flattenedItems.find(n => n.view === currentView)?.label || (currentView === AppView.PROFILE ? 'Mon Profil' : 'Biso Peto')}</h1></div>
                        ...
                        <div className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-2xl border transition-all ${syncQueueSize > 0 ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 border-orange-100' : 'bg-green-50 dark:bg-green-900/20 text-green-600 border-green-100'}`}>
                            {syncQueueSize > 0 ? (
                                <>
                                    ...
                                    <span className="text-xs font-black uppercase tracking-widest">{syncQueueSize} en attente</span>
                                </>
                            ) : isOnline ? (
                                <>
                                    <Cloud className="w-4 h-4" />
                                    <span className="text-xs font-black uppercase tracking-widest">Synchronisé</span>
                                </>
                            ) : (
                                <>
                                    <CloudOff className="w-4 h-4 text-gray-400" />
                                    <span className="text-xs font-black uppercase tracking-widest text-gray-400">Mode Local</span>
                                </>
                            )}
                        </div>
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <div className="hidden md:block text-left mr-2">
                                    ...
                                    <p className={`text-xs font-black uppercase mt-0.5 tracking-tighter ${currentView === AppView.PROFILE ? 'text-white/70' : 'text-gray-500'}`}>{user?.type}</p>
                                </div>
                            </button>
                        </div>
                    </div>
