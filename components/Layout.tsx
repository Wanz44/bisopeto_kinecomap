            ...
            <aside className="hidden md:flex flex-col w-72 h-[calc(100vh-3rem)] m-6 rounded-[2.5rem] bg-white/95 dark:bg-[#111827]/95 backdrop-blur-2xl border border-gray-100 dark:border-white/5 shadow-2xl transition-all duration-300 relative z-50 shrink-0">
                <div className="p-8 flex flex-col gap-1">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div className="flex flex-col"><span className="font-black text-2xl text-primary tracking-tighter leading-none">KIN ECO MAP</span><span className="text-[9px] font-bold text-secondary uppercase tracking-[0.1em] mt-0.5">BISO PETO GROUP</span></div>
                    </div>
                </div>
                <nav className="flex-1 px-4 space-y-6 mt-2 overflow-y-auto no-scrollbar">
                    {navSections.map((section, idx) => (
                        <div key={idx} className="space-y-1.5">
                            {section.title && <h3 className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">{section.title}</h3>}
                            {section.items.map((item) => {
                                const isActive = currentView === item.view;
                                return (
                                    <button key={item.view} onClick={() => onChangeView(item.view)} className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all duration-300 group relative ${isActive ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5'}`}>
                                        ...
                                        {item.view === AppView.NOTIFICATIONS && unreadNotifications > 0 && !isActive && <span className="ml-auto w-5 h-5 bg-action text-gray-900 text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white dark:border-gray-900">{unreadNotifications}</span>}
                                    </button>
                                );
                            })}
                        </div>
